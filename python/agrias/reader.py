...
DEFAULT_MERIAN_DIR = '/Users/kadofong/work/surveys/merian/agrias/'

def mk_geav (dirstem=None):
    if dirstem is None:
...
def merianbroad (filename=None, zphot=0.08, dirstem=None):
    if dirstem is None:
        dirstem = DEFAULT_MERIAN_DIR + '/local_data/inputs/'    
    if filename is None:
        filename = f'{dirstem}/Merian_DR1_photoz_EAZY_v2.0.fits'
...
def merianselect (
    ...
    if merian is None:
        ...
        if version == 2:
            merian = pd.read_csv(f'{dirstem}/scratch_catalogs/Merian_DR1_photoz_EAZY_v2.0_pbandgt0.1.csv')
            merian['logmass_gaap'] = merian['logmass_gaap1p0']
        elif version == 3:
            merian =  table.Table(fits.getdata(f'{dirstem}/Merian_DR1_photoz_EAZY_v3.0_inbandspecz.fits',1)).to_pandas()
...
def galexcrossmatch ( filename=None,  dirstem=None):
    if dirstem is None:
        dirstem = DEFAULT_MERIAN_DIR + '/local_data/inputs/'          
    if filename is None:
        filename = f'{dirstem}/MAST_Crossmatch_GALEX.csv'
...
def get_meriancrossgalex (merian=None, dirstem=None):
    if dirstem is None:
        dirstem = DEFAULT_MERIAN_DIR + '/local_data/inputs/'          
    if merian is None:
        merian = table.Table(fits.getdata(f'{dirstem}/Merian_DR1_photoz_EAZY_v1.2.fits',1))
